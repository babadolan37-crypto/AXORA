...
import { IncomeEntry } from '../types/accounting';
...
interface IncomeSheetProps {
  ...
  onUpdateEntry: (id: string, entry: Omit<IncomeEntry, 'id'>) => void;
  onDeleteEntry: (id: string) => void;
}

export function IncomeSheet({ entries, incomeSources, paymentMethods, onAddEntry, onUpdateEntry, onDeleteEntry }: IncomeSheetProps) {
    ...
    <>
      ...
      {isFormOpen && (
        <div className="bg-white border border-gray-200 rounded-lg p-6 shadow-sm">
          <h3 className="mb-4">{editingId ? 'Edit' : 'Tambah'} Pemasukan</h3>
          <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-sm text-gray-600 mb-1">Sumber Pemasukan</label>
              <select
            ...
            <div>
              <label className="block text-sm text-gray-600 mb-1">Jumlah Pemasukan</label>
              <input
                type="number"
            ...
            <div className="md:col-span-2">
              <label className="block text-sm text-gray-600 mb-1">Siapa yang Bayar</label>
              <input
                type="text"
            ...
            <div className="md:col-span-2">
              ...
              <div className="space-y-3">
                <div className="flex items-center gap-2">
                  <label className="flex items-center gap-2 bg-green-50 text-green-700 px-4 py-2 rounded-lg hover:bg-green-100 cursor-pointer transition-colors border border-green-300">
                    <Camera size={20} />
                    Upload Foto
                    <input
                      type="file"
          ...
          <table className="w-full">
            <thead className="bg-gray-50 border-b border-gray-200">
              <tr>
                ...
                <th className="px-4 py-3 text-left text-sm text-gray-600">Sumber</th>
              ...
              {entries.length === 0 ? (
                <tr>
                  <td colSpan={8} className="px-4 py-8 text-center text-gray-500">
                    Belum ada data pemasukan. Klik tombol "Tambah Pemasukan" untuk menambah data.
                  </td>
                </tr>
              ) : (
                entries.map((entry) => (
                  <tr key={entry.id} className="border-b border-gray-100 hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3 text-sm text-right text-green-600">{formatCurrency(entry.amount)}</td>
